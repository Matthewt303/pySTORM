...
@jit(nopython=True, nogil=True, cache=False)
def nelder_mead_sigma(
    obj_func, x0: "np.ndarray[np.float32]", spot: tuple, max_iter: int, tol: float
...
def mlefit_nm_sigma(spot, eps, max_it):
    ...
    theta_opt = nelder_mead_sigma(
        neg_log_likelihood_sigma, theta0, (spot,), max_it, eps
    )
...
def calc_loc_prec(spot, theta_opt):
    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            dmodel_dt[0] = derivative_gaussian_integral(
                j, theta_opt[0], theta_opt[4], theta_opt[2], PSFy
            )
            dmodel_dt[1] = derivative_gaussian_integral(
                i, theta_opt[1], theta_opt[4], theta_opt[2], PSFx
            )

            dmodel_dt[4] = derivative_gaussian_integral_2d_sigma(
                j, i, theta_opt[0], theta_opt[1], theta_opt[4], theta_opt[2], PSFx, PSFy
            )
...
def convert_theta_xy(
    theta_xy: "np.ndarray", coords: list, roi_size: int, pix_res: float
):
...
@jit(nopython=True, nogil=True, cache=False)
def convert_to_photons(
    data: "np.ndarray", adu: float, gain: int, offset: float
) -> "np.ndarray":
...
def localise_frame(
    ...
    ids = np.linspace(id_num, id_num + theta.shape[0] - 1, theta.shape[0]).reshape(
        -1, 1
    )
    ...
    intensity_ph = convert_to_photons(theta[:, 2], adu, gain, offset).reshape(-1, 1)

    bg_ph = convert_to_photons(theta[:, 3], adu, gain, offset).reshape(-1, 1)

    sigma_nm = 1000 * convert_pix_to_um(theta[:, 4], pix_res=pix_res)

    sigma_nm = sigma_nm.reshape(-1, 1)

    loc_prec_nm = 1000 * convert_pix_to_um(loc_prec, pix_res=pix_res).reshape(-1, 1)

    loc_table = np.hstack(
        (ids, frame_column, xy_locs, sigma_nm, intensity_ph, bg_ph, loc_prec_nm)
    )
